    ...
    public static function fromJsonValue(mixed $value): static {
        ...
        if (!is_array($value)) {
            throw new InvalidArgumentException(
                'Expected object for XdrPaymentOperation JSON value, got ' . get_debug_type($value)
            );
        }
        XdrJsonHelper::rejectUnknownFields($value, ['destination', 'asset', 'amount'], 'XdrPaymentOperation');
        if (!array_key_exists('destination', $value)) {
            throw new InvalidArgumentException(
                'Missing required field destination for XdrPaymentOperation'
            );
        }
        $destination = XdrMuxedAccount::fromJsonValue($value['destination']);
        if (!array_key_exists('asset', $value)) {
            throw new InvalidArgumentException(
                'Missing required field asset for XdrPaymentOperation'
            );
        }
        $asset = XdrAsset::fromJsonValue($value['asset']);
        if (!array_key_exists('amount', $value)) {
            throw new InvalidArgumentException(
                'Missing required field amount for XdrPaymentOperation'
            );
        }
