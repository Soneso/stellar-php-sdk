<?php declare(strict_types=1);

// This file was automatically generated by xdrgen.
// DO NOT EDIT or your changes may be overwritten.

namespace Soneso\StellarSDK\Xdr;

use InvalidArgumentException;
use JsonException;

class XdrContractEventBodyV0 {

    public array $topics;
    public XdrSCVal $data;

    public function __construct(array $topics, XdrSCVal $data) {
        $this->topics = $topics;
        $this->data = $data;
    }

    public function encode(): string {
        $topicsCount = count($this->topics);
        $bytes = XdrEncoder::integer32($topicsCount);
        foreach ($this->topics as $topicsItem) {
            $bytes .= $topicsItem->encode();
        }
        $bytes .= $this->data->encode();
        return $bytes;
    }

    public static function decode(XdrBuffer $xdr): XdrContractEventBodyV0 {
        $topics = [];
        $topicsSize = $xdr->readInteger32();
        for ($i = 0; $i < $topicsSize; $i++) {
            $topics[] = XdrSCVal::decode($xdr);
        }
        $data = XdrSCVal::decode($xdr);
        return new XdrContractEventBodyV0($topics, $data);
    }

    public function getTopics(): array { return $this->topics; }
    public function setTopics(array $topics): void { $this->topics = $topics; }
    public function getData(): XdrSCVal { return $this->data; }
    public function setData(XdrSCVal $data): void { $this->data = $data; }

    public function toBase64Xdr(): string {
        return base64_encode($this->encode());
    }

    public static function fromBase64Xdr(string $xdr): static {
        $decoded = base64_decode($xdr, true);
        if ($decoded === false) {
            throw new InvalidArgumentException('Invalid base64-encoded XDR');
        }
        return static::decode(new XdrBuffer($decoded));
    }

    public function toJsonValue(): array {
        return [
            'topics' => array_map(static function ($item) { return $item->toJsonValue(); }, $this->topics),
            'data' => $this->data->toJsonValue(),
        ];
    }

    public static function fromJsonValue(mixed $value): static {
        if (is_array($value) && array_key_exists('$schema', $value)) {
            unset($value['$schema']);
        }
        if (!is_array($value)) {
            throw new InvalidArgumentException(
                'Expected object for XdrContractEventBodyV0 JSON value, got ' . get_debug_type($value)
            );
        }
        XdrJsonHelper::rejectUnknownFields($value, ['topics', 'data'], 'XdrContractEventBodyV0');
        if (!array_key_exists('topics', $value)) {
            throw new InvalidArgumentException(
                'Missing required field topics for XdrContractEventBodyV0'
            );
        }
        $topics = (static function ($v) {
            if (!is_array($v)) {
                throw new InvalidArgumentException('Expected JSON array, got ' . get_debug_type($v));
            }
            $out = [];
            foreach ($v as $item) { $out[] = XdrSCVal::fromJsonValue($item); }
            return $out;
        })($value['topics']);
        if (!array_key_exists('data', $value)) {
            throw new InvalidArgumentException(
                'Missing required field data for XdrContractEventBodyV0'
            );
        }
        $data = XdrSCVal::fromJsonValue($value['data']);
        return new static($topics, $data);
    }

    /**
     * @throws JsonException If the value contains structures that cannot be encoded as JSON.
     */
    public function toJson(): string {
        return json_encode(
            $this->toJsonValue(),
            JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE
        );
    }

    /**
     * @throws JsonException If $json is not syntactically valid JSON.
     * @throws InvalidArgumentException If an object in $json repeats a key, or if
     *         the JSON shape does not match this type.
     */
    public static function fromJson(string $json): static {
        return static::fromJsonValue(XdrJsonHelper::decodeText($json));
    }
}
