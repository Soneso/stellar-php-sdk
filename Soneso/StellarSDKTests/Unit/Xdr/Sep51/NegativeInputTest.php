 ...
 * Two batteries:
 *
 *   (1) Per-union per-shape rejection methods. For each union class flagged
 *       with a `// @sep51-union <Class> shape=<shape>` marker by the
 *       generator, four or five named methods exercise the rejection paths
 *       in fromJsonValue per the SEP-51 negative-gate contract:
 *
 ...
 *
 *       Each method asserts InvalidArgumentException and is named exactly as
 *       the negative_gate.sh shell script expects so the gate can verify
 *       presence by grep.
 *
 *   (2) XdrJsonHelper malformed-input rejection methods. Every public helper
 *       gets an adversarial battery covering the documented rejection paths:
 *       escapeString / unescapeString round-trip on malformed escape
 *       sequences; stringToInt128Parts / stringToInt256Parts on out-of-range
 *       and non-digit input; hexToBytes on non-hex / odd-length / uppercase.
 */
class NegativeInputTest extends TestCase
{
    ...
    // =========================================================================
    // XdrJsonHelper malformed-input rejection battery
    // =========================================================================

    public function testEscapeStringRoundTripsThroughUnescape(): void
    {
        // Reflexive: escapeString output must round-trip back to the input.
        $cases = [
            'plain ASCII',
            "with\ttab",
            "with\nlf",
            "with\rcr",
            "with backslash \\ literal",
            "binary \x00\x01\x02 prefix",
            "high \xff bytes \x80",
            '',
        ];
        foreach ($cases as $input) {
            $escaped = XdrJsonHelper::escapeString($input);
            $back = XdrJsonHelper::unescapeString($escaped);
            $this->assertSame($input, $back, "Round-trip failed for: " . bin2hex($input));
        }
    }

    public function testUnescapeStringRejectsTrailingBackslash(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('trailing backslash');
        XdrJsonHelper::unescapeString('hello\\');
    }

    public function testUnescapeStringRejectsTruncatedHexEscape(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('truncated');
        XdrJsonHelper::unescapeString('hello\\x');
    }

    public function testUnescapeStringRejectsTruncatedHexEscapeAtBoundary(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('truncated');
    ...
    public function testUnescapeStringRejectsUppercaseHexEscape(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('lowercase hex');
        XdrJsonHelper::unescapeString('hello\\xFF');
    }

    public function testUnescapeStringRejectsNonHexDigitsInEscape(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('lowercase hex');
        XdrJsonHelper::unescapeString('hello\\xgg');
    }

    public function testUnescapeStringRejectsUnknownEscapeLetter(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('unrecognised escape');
        XdrJsonHelper::unescapeString('hello\\q');
    }
    ...
    public function testHexToBytesRejectsOddLength(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('even length');
        XdrJsonHelper::hexToBytes('abc');
    }

    public function testHexToBytesRejectsUppercase(): void
    {
        $this->expectException(\InvalidArgumentException::class);
    ...
    public function testHexToBytesRejectsMixedCase(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('lowercase');
        XdrJsonHelper::hexToBytes('aB');
    ...
    public function testHexToBytesRejectsNonHexCharacters(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('lowercase');
    ...
    public function testHexToBytesRejectsTrailingNewline(): void
    {
        // The implementation uses \A and \z anchors so a trailing \n must
        // not bypass the lowercase-hex check.
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::hexToBytes("ab\n");
    }

    public function testHexToBytesAcceptsEmpty(): void
    {
        $this->assertSame('', XdrJsonHelper::hexToBytes(''));
    }

    public function testHexToBytesAcceptsLowercase(): void
    {
        $this->assertSame("\xab\xcd", XdrJsonHelper::hexToBytes('abcd'));
    }
    ...
    public function testStringToInt128PartsRejectsEmpty(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToInt128Parts('');
    }

    public function testStringToInt128PartsRejectsNonDigit(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToInt128Parts('not_a_number');
    }

    public function testStringToInt128PartsRejectsScientificNotation(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToInt128Parts('1e10');
    }

    public function testStringToInt128PartsRejectsLeadingPlus(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToInt128Parts('+1');
    }

    public function testStringToInt128PartsRejectsAboveRange(): void
    {
        // 2^127 is one above the max signed 128-bit integer.
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('out of the range');
        XdrJsonHelper::stringToInt128Parts('170141183460469231731687303715884105728');
    }

    public function testStringToInt128PartsRejectsBelowRange(): void
    {
        // -2^127 - 1 is one below the min signed 128-bit integer.
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('out of the range');
        XdrJsonHelper::stringToInt128Parts('-170141183460469231731687303715884105729');
    }

    public function testStringToInt128PartsAcceptsZero(): void
    {
        $parts = XdrJsonHelper::stringToInt128Parts('0');
        $this->assertSame(['hi' => '0', 'lo' => '0'], $parts);
    }
    ...
    public function testStringToInt256PartsRejectsEmpty(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToInt256Parts('');
    }

    public function testStringToInt256PartsRejectsNonDigit(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToInt256Parts('garbage');
    }

    public function testStringToInt256PartsRejectsAboveRange(): void
    {
        // 2^255
        $bound = '57896044618658097711785492504343953926634992332820282019728792003956564819968';
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('out of the range');
        XdrJsonHelper::stringToInt256Parts($bound);
    }

    public function testStringToInt256PartsRejectsBelowRange(): void
    {
        // -2^255 - 1
        $bound = '-57896044618658097711785492504343953926634992332820282019728792003956564819969';
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('out of the range');
        XdrJsonHelper::stringToInt256Parts($bound);
    }
    ...
    // -------------------------------------------------------------------------
    // stringToUint128Parts / stringToUint256Parts
    // -------------------------------------------------------------------------

    public function testStringToUint128PartsRejectsNegative(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToUint128Parts('-1');
    }

    public function testStringToUint128PartsRejectsAboveRange(): void
    {
        // 2^128
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('out of the range');
    ...
    public function testStringToUint256PartsRejectsNegative(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::stringToUint256Parts('-1');
    }

    public function testStringToUint256PartsRejectsAboveRange(): void
    {
        // 2^256
        $this->expectException(\InvalidArgumentException::class);
        $this->expectExceptionMessage('out of the range');
        XdrJsonHelper::stringToUint256Parts(
    ...
    public function testCanonicalJsonRejectsMalformedInput(): void
    {
        $this->expectException(\JsonException::class);
        XdrJsonHelper::canonicalJson('{not valid json}');
    }

    public function testCanonicalJsonRejectsTruncatedInput(): void
    {
        $this->expectException(\JsonException::class);
    ...
    public function testWrapUnsignedToSignedIntRejectsEmpty(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::wrapUnsignedToSignedInt('');
    }

    public function testWrapUnsignedToSignedIntRejectsNegativeSign(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        XdrJsonHelper::wrapUnsignedToSignedInt('-1');
    }

    public function testWrapUnsignedToSignedIntRejectsAboveTwoToThe64(): void
    {
        $this->expectException(\InvalidArgumentException::class);
