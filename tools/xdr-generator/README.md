...
- **`arm_storage_overrides.rb`** — `ARM_STORAGE_OVERRIDES` supplies the `encode`/`decode`/`toTxRep`/`fromTxRep` emission for union arms whose PHP field holds something other than the raw bytes its XDR declaration gives (e.g. `XdrSCAddress` keeps the contract id as hexadecimal). `ARM_STORAGE_HELPERS` emits the class members that emission calls, so the canonicalisation has one implementation the generated readers and the hand-written wrapper share.
