...
The PHP SDK implements 18 Stellar Ecosystem Proposals (SEPs) that cover authentication, asset transfers, identity verification, and other standardized protocols for integrating with anchors and other Stellar services.
...
$toml = StellarToml::fromDomain('anchor.example.com');
...
$webAuth = WebAuth::fromDomain('anchor.example.com', $network);
$jwt = $webAuth->jwtToken($clientKeyPair->getAccountId(), [$clientKeyPair]);
```
...
$service = InteractiveService::fromDomain('anchor.example.com');
